...
describe("undersized diagram audit repair", () => {
  ...
  it("keeps diagram-required records without an original source asset out of learner play without mutating their content", () => {
    ...
    expect(reconciliation.learnerFacingLinkedMappings).toEqual({ total: 128, bySubject: { Biology: 75, Chemistry: 28, Physics: 25, "Use of English": 0 } });
    expect(reconciliation.missingOriginalVisualHolds).toMatchObject({ total: 3, bySubject: { Biology: 2, Chemistry: 1 }, learnerState: "excluded by the production eligibility guard; no generated replacement is used" });
    expect(reconciliation.safetyBoundary).toContain("The 42 reviewed screenshot mappings and the 3 remaining missing-original holds are distinct sets; the latter are not learner-visible while their original source figures are unavailable.");
  });
  ...
  it("repairs biology_0505 only through a guarded byte-identical organism panel and A/I correction", () => {
    const script = readFileSync(resolve(import.meta.dirname, "../scripts/repairBiology0505ChitinousExoskeletonRecord.mjs"), "utf8");
    expect(script).toContain('externalId: "biology_0505"');
    expect(script).toContain("biology-0502-testdriller-original_eea7904a.png");
    expect(script).toContain("byte-identical");
    expect(script).toContain("currentAnswerIndex: 2");
    expect(script).toContain("replacementAnswerIndex: 0");
  ...
  it("repairs biology_0523 only through a guarded clean water-reabsorption panel and B/III-and-IV correction", () => {
    const script = readFileSync(resolve(import.meta.dirname, "../scripts/repairBiology0523WaterReabsorptionRecord.mjs"), "utf8");
    expect(script).toContain('externalId: "biology_0523"');
    expect(script).toContain("biology-0523-testdriller-original_f9b78c24.png");
    expect(script).toContain("currentAnswerIndex: 2");
    expect(script).toContain("replacementAnswerIndex: 1");
    expect(script).toContain("UPDATE questionItems SET answerIndex = ?, explanation = ?, diagramUrl = ?");
    expect(script).toContain("immutableFields.every");
  ...
  it("releases biology_0527 only through a guarded clean thermoregulation graph while preserving the source-matching B/I-independent content", () => {
    const script = readFileSync(resolve(import.meta.dirname, "../scripts/releaseRecoveredBiology0527Diagram.mjs"), "utf8");
    expect(script).toContain('externalId: "biology_0527"');
    expect(script).toContain("biology-0527-testdriller-original_8ece4a3c.png");
    expect(script).toContain("protectedFields.every");
    expect(script).toContain("UPDATE questionItems SET diagramUrl = ?");
    expect(script).not.toContain("SET questionText");
    expect(script).not.toContain("SET optionsJson");
  ...
  it("releases biology_0890 only through a guarded clean breeding-posture figure while preserving the source-matching B/amplexus content", () => {
    const script = readFileSync(resolve(import.meta.dirname, "../scripts/releaseRecoveredBiology0890Diagram.mjs"), "utf8");
    expect(script).toContain('externalId: "biology_0890"');
    expect(script).toContain("biology-0890-testdriller-original_300cb4b6.png");
    expect(script).toContain("protectedFields.every");
    expect(script).toContain("UPDATE questionItems SET diagramUrl = ?");
    expect(script).not.toContain("SET questionText");
    expect(script).not.toContain("SET optionsJson");
  ...
  it("releases biology_1041 only through a guarded clean repeated-original frog water-loss figure while preserving protected 2020 learner content", () => {
    const script = readFileSync(resolve(import.meta.dirname, "../scripts/releaseRecoveredBiology1041Diagram.mjs"), "utf8");
    expect(script).toContain('externalId: "biology_1041"');
    expect(script).toContain("biology-1041-testdriller-original_f360a5e8.png");
    expect(script).toContain("protectedFields.every");
    expect(script).toContain("UPDATE questionItems SET diagramUrl = ?");
    expect(script).toContain("preserves all protected learner content");
    expect(script).not.toContain("SET questionText");
    expect(script).not.toContain("SET optionsJson");
    expect(script).not.toContain("SET answerIndex");
  });

  it("releases biology_1033 only through a guarded clean countershading original while preserving the source-matching C content", () => {
    const script = readFileSync(resolve(import.meta.dirname, "../scripts/releaseRecoveredBiology1033Diagram.mjs"), "utf8");
    expect(script).toContain('externalId: "biology_1033"');
    expect(script).toContain("biology-1033-testdriller-original_425154e0.png");
    expect(script).toContain("protectedFields.every");
    expect(script).toContain("UPDATE questionItems SET diagramUrl = ?");
    expect(script).not.toContain("SET questionText");
    expect(script).not.toContain("SET optionsJson");
  ...
  it("releases biology_1111 only through a guarded clean repeated-original F2 figure while preserving protected 2025 learner content", () => {
    const script = readFileSync(resolve(import.meta.dirname, "../scripts/releaseRecoveredBiology1111Diagram.mjs"), "utf8");
    expect(script).toContain('externalId: "biology_1111"');
    expect(script).toContain("boundary-x0-520-y38-354_5d0f5769.png");
    expect(script).toContain("protectedFields.every");
    expect(script).toContain("UPDATE questionItems SET diagramUrl = ?");
    expect(script).toContain("preserves all protected learner content");
    expect(script).not.toContain("SET questionText");
