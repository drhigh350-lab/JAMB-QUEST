...
const reportPath = path.join(projectRoot, "reports", "recovered_biology_0905_diagram_release_20260825.json");
const target = {
  externalId: "biology_0905",
  expectedCurrentDiagramUrl: null,
  replacementDiagramUrl: "/manus-storage/biology-0905-schoolngr-original_705e2a37.png",
  questionText: "Use the diagram shown to answer the question. The experiment above is used to demonstrate the process of",
  optionsJson: '["transportation","water culture","diffusion","osmosis"]',
  answerIndex: 3,
  topic: "Transport",
  sourceId: 22500001,
};
...
try {
  ...
  if (!sourceMatches) {
    ...
    receipt.reason = "already mapped to verified original thistle-funnel apparatus";
    ...
    if (result.affectedRows !== 1) {
      ...
      receipt.reason = "exact SchoolNGR original thistle-funnel apparatus verified separately from the source answer and free of watermark, options, answer letters, correction text, and source chrome";
    }
  }
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify({
      ...
      scope: "Guarded mapping-only release of an exact-source Biology osmosis apparatus. Question text, options, answer index, topic, explanation, and source relationship remain unchanged.",
      target,
      receipt,
    }, null, 2)}\n`,
  );
