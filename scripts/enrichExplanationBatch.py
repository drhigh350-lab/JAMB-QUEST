...
print(json.dumps({"model": MODEL, "input": len(items), "output": len(results), "ready": sum(1 for result in results if result["quality_gate"] and not result["needs_review"]), "needs_review": sum(1 for result in results if result["needs_review"])}, indent=2))
