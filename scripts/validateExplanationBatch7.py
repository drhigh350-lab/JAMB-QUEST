...
AUDIT = ROOT / "reports/explanations_batch7_literal_audit.json"
PENDING = Path("/home/ubuntu/upload/JBquestions(1).csv")
MODEL_BANK = Path("/home/ubuntu/webdev-static-assets/jamb_high_yield_practice_bank_1000_explanations_batches2_6_v1.json")
OUTPUT = ROOT / "reports/explanations_batch7_compatibility_audit.json"
...
def main() -> None:
    ...
    bank_records = records(json.loads(MODEL_BANK.read_text(encoding="utf-8")))
    by_id = {str(record.get("id") or record.get("record_id")): record for record in bank_records}
