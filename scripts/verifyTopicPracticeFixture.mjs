...
try {
  ...
  const startButton = panel.getByRole("button", { name: /Start .*drill/i });
  ...
  await page.getByTestId("topic-drill-count-50").click();
  await startButton.click();
  const longBroadConfig = JSON.parse((await page.getByTestId("fixture-launched-config").textContent({ timeout: 10_000 })) || "{}");
  if (longBroadConfig.count !== 50 || !Array.isArray(longBroadConfig.topics) || !groupTopics.every((topic) => longBroadConfig.topics.includes(topic))) throw new Error("A selected broad area did not preserve the learner-selected 50-question drill configuration.");
  ...
  if (result.count !== 50) throw new Error(`Expected the chosen 50-question drill, received ${result.count ?? "no count"}.`);
