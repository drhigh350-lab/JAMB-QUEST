...
const inputPaths = [
  "/home/ubuntu/jamb-import-staging/chemistry_past_questions_aug15_part3_51_75.validated.json",
  "/home/ubuntu/jamb-import-staging/chemistry_past_questions_aug15_part4_76_100.validated.json",
];
const sourceLabel = "JAMB Quest Chemistry Bank · Trusted owner batch · August 2026 · Questions 51–100";
const permissionNote = "Trusted owner-provided Chemistry questions 51–100, supplied in two parts on 15 August 2026. Directly released at the owner’s instruction after automatic duplicate and card-shape safeguards.";
const normalise = (value: string) => value.toLowerCase().replace(/[^a-z0-9]+/g, "");
const metadataPattern = /(?:✓|©|\bcorrect\s+answer\s*:|\bexplanation\s*:|\bwhy\s+others?\s+are\s+wrong\s*:)/i;

type IncomingRecord = { externalId: string; subject: "Chemistry"; topic: string; difficulty: "easy" | "medium" | "hard"; question: string; options: string[]; answerIndex: number; explanation?: string };
...
const existingRows = await db.select({ subject: questionItems.subject, question: questionItems.questionText }).from(questionItems).where(eq(questionItems.subject, "Chemistry"));
...
if (release.length !== 50) throw new Error(`Expected 50 new unique Chemistry questions; received ${release.length} after duplicate protection.`);
...
if (!sourceId) {
  const payload = authorisedImportSchema.parse({
    sourceLabel,
    permissionNote,
    fileName: "chemistry_past_questions_aug15_parts_51_to_100.txt",
    storageKey: "owner-markdown://chemistry_past_questions_aug15_parts_51_to_100.txt",
...
await writeFile("/home/ubuntu/jamb-import-staging/trusted_chemistry_aug15_questions_51_to_100_import_receipt.json", `${JSON.stringify(receipt, null, 2)}\n`);
