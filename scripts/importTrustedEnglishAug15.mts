...
const inputPaths = [
  "/home/ubuntu/jamb-import-staging/english_past_questions_aug15_part1_1_25.validated.json",
  "/home/ubuntu/jamb-import-staging/english_past_questions_aug15_part2_26_50.validated.json",
  "/home/ubuntu/jamb-import-staging/english_past_questions_aug15_part3_51_75.validated.json",
  "/home/ubuntu/jamb-import-staging/english_past_questions_aug15_part4_76_100.validated.json",
];
const sourceLabel = "JAMB Quest Use of English Bank · Trusted owner batch · August 2026 · Questions 1–100";
const permissionNote = "Trusted owner-provided Use of English questions 1–100, supplied in four parts on 15 August 2026. Directly released at the owner’s instruction after automatic duplicate and card-shape safeguards.";
const normalise = (value: string) => value.toLowerCase().replace(/[^a-z0-9]+/g, "");
const metadataPattern = /(?:✓|©|\bcorrect\s+answer\s*:|\bexplanation\s*:|\bwhy\s+others?\s+are\s+wrong\s*:)/i;

type IncomingRecord = { externalId: string; subject: "Use of English"; topic: string; difficulty: "easy" | "medium" | "hard"; question: string; options: string[]; answerIndex: number; explanation?: string };
...
const existingRows = await db.select({ subject: questionItems.subject, question: questionItems.questionText }).from(questionItems).where(eq(questionItems.subject, "Use of English"));
...
if (release.length !== 100) throw new Error(`Expected 100 new unique Use of English questions; received ${release.length} after duplicate protection.`);
...
if (!sourceId) {
  const payload = authorisedImportSchema.parse({
    sourceLabel,
    permissionNote,
    fileName: "english_past_questions_aug15_parts_1_to_100.txt",
    storageKey: "owner-markdown://english_past_questions_aug15_parts_1_to_100.txt",
...
await writeFile("/home/ubuntu/jamb-import-staging/trusted_english_aug15_questions_1_to_100_import_receipt.json", `${JSON.stringify(receipt, null, 2)}\n`);
