...
if (!enrichmentPath || !queuePath) throw new Error("Usage: pnpm tsx scripts/importApprovedSubmittedRichEnrichments.mts <enrichment-output-json> <queue-input-json>");
const enrichments = JSON.parse(await readFile(enrichmentPath, "utf8")) as Array<{ id: string; subject: string; lines: string[]; quality_gate: boolean; needs_review: boolean }>;
...
const approved = enrichments.filter((record) => record.quality_gate && !record.needs_review && record.lines.length === 6);
...
for (const enrichment of approved) {
  ...
  if (!sourceRecord) throw new Error(`Missing queued source record for ${enrichment.id}`);
  ...
  await db.insert(questionItems).values({
    ...
    explanation: enrichment.lines.join("\n"),
...
console.log(JSON.stringify({ verified: true, readyToImport: approved.length, inserted, skippedExisting, modelHeldForReview: enrichments.length - approved.length, sourceAmbiguitiesStillHeld: 8 }, null, 2));
