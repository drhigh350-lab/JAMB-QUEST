...
try {
  ...
  const metrics = await page.evaluate(() => ({
    ...
    drillButtons: Array.from(document.querySelectorAll(".weak-topic-list button")).filter((button) => button.textContent?.includes("Fix in 20")).length,
  ...
  if (metrics.chartBars !== 3 || metrics.logRows !== 3 || metrics.weakTopics !== 2 || metrics.drillButtons !== 2 || metrics.logReviewButtons !== 3 || metrics.savedQuestions !== 2 || metrics.revisionSteps !== 3 || !metrics.comparisonVisible || metrics.overflow > 1 || !metrics.text.includes("NEXT-STUDY PLAN") || !metrics.text.includes("72%") || !metrics.text.includes("Genetics 33%")) throw new Error(`Progress analytics fixture failed: ${JSON.stringify(metrics)}`);
