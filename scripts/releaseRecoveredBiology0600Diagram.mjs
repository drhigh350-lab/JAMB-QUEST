...
const reportPath = path.join(projectRoot, "reports", "recovered_biology_0600_diagram_release_20260825.json");
const target = {
  externalId: "biology_0600",
  expectedCurrentDiagramUrl: null,
  replacementDiagramUrl: "/manus-storage/biology-0600-schoolngr-original_413d60ab.png",
  questionText: "Use the diagram above to answer this question. The gas evolved in the process is",
  optionsJson: '["carbon (IV) oxide","nitrogen","oxygen","carbon (II) oxide"]',
  answerIndex: 0,
  topic: "Respiration",
  sourceId: 22500001,
};
...
try {
  ...
  if (!sourceMatches) {
    ...
    receipt.reason = "already mapped to verified original fermentation apparatus";
    ...
    if (result.affectedRows !== 1) {
      ...
      receipt.reason = "exact SchoolNGR original fermentation apparatus verified separately from the source answer and free of watermark, options, answer letters, correction text, and source chrome";
    }
  }
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify({
      ...
      scope: "Guarded mapping-only release of an exact-source Biology apparatus. Question text, options, answer index, topic, explanation, and source relationship remain unchanged.",
      target,
      receipt,
    }, null, 2)}\n`,
  );
