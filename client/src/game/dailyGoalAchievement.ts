...
export function downloadDailyGoalAchievement(achievement: DailyGoalAchievement): void {
  ...
  const blob = new Blob([buildDailyGoalAchievementSvg(achievement)], { type: "image/svg+xml;charset=utf-8" });
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `jamb-quest-goal-complete-${(achievement.dateKey || "today").replace(/[^a-z0-9-]/gi, "-")}.svg`;
