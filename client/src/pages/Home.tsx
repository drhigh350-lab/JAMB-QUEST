  ...
  return <main className={`home-page tabbed-home compact-home ${entranceReady ? "entrance-ready" : ""}`}>
    ...
    <div key={`content-${activeTab}`} className="tab-content page-shell tab-cinematic-entry tab-cinematic-content">
      ...
      {activeTab === "profile" && <>
        ...
        {isOwner && <details className="owner-diagram-desk tab-section"><summary><span><b>HELD DIAGRAM DESK</b><small>{ownerHeldDiagramRecords.length} figure-dependent records</small></span><ArrowRight size={16} /></summary><p className="owner-diagram-intro">Read-only evidence queue. These records remain outside the learner bank until an exact owner-original visual is attached.</p><div className="owner-diagram-list">{ownerHeldDiagramRecords.length ? ownerHeldDiagramRecords.map((record) => <article key={record.id}><div><b>{record.subject} · {record.topic}</b><small>Record {record.id} · {record.externalId}</small><p>{record.questionPreview}</p></div><em>Held — awaiting owner-original visual</em></article>) : <p className="compact-empty">No figure-dependent records are currently held without an owner-original visual.</p>}</div></details>}
