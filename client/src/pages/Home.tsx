...
export default function Home({ loading, loadError, progress, canReview, onRetryLoad, onStart, auth, questionCount, questionCountReady, comeback, reminder, achievementStats, examHistory, onOpenExamLog = () => undefined, examReviewOpening = false, examReviewError = null, weakTopics, subjectPerformance = [], fullMockSubjectPerformance = [], availableTopics = [], bookmarks = [], comparison, onUpdateDailyMinimum, onUpdateDailyGoal = () => undefined, onEnablePush, onDisablePush, onTestPush = () => undefined, onRefreshProviderQueue = () => undefined, queueRefreshing = false, pushWorking, pushStatus, pwa, resumableCbt = null, onResumeCbt = () => undefined, onDiscardResumableCbt = () => undefined }: HomeProps) {
    ...
    <div key={`content-${activeTab}`} className="tab-content page-shell tab-cinematic-entry tab-cinematic-content">
      {activeTab === "practice" && <>
        ...
        <section className="compact-action-grid tab-section entrance-item entrance-tools" aria-label="Practice tools">
          ...
          <CompactPanel eyebrow="03 / TOPIC" title="Study by official JAMB syllabus area" note="Launch a broad syllabus section or choose one exact official area; unavailable areas stay visible until questions are loaded" tone="maize">
