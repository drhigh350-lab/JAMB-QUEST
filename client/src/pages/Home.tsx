...
interface HomeProps {
  ...
  ownerHeldDiagramRecords?: Array<{ id: number; externalId: string; subject: string; topic: string; questionPreview: string }>;
  ...
  return <main className={`home-page tabbed-home compact-home ${entranceReady ? "entrance-ready" : ""}`}>
    ...
    <div key={`content-${activeTab}`} className="tab-content page-shell tab-cinematic-entry tab-cinematic-content">
      {activeTab === "practice" && <>
        ...
        <section className="compact-action-grid tab-section entrance-item entrance-tools" aria-label="Practice tools">
          <CompactPanel eyebrow="01 / TODAY" title="Today’s JAMB practice" note={`${selectedState.today.questionsAnswered} of ${selectedState.dailyGoalCount} questions${selectedState.dailyGoalTopic ? ` · ${dailyGoalTopicLabel}` : " today"}`} defaultOpen tone="ink">
            ...
            {selectedState.today.completedMinimum && <section className="daily-completion-summary" data-testid="daily-completion-summary" aria-label="Today's completed study summary"><span>DAILY PLAN COMPLETE</span><b>{selectedState.today.correctCount}/{selectedState.today.questionsAnswered} correct · {selectedState.today.questionsAnswered ? Math.round((selectedState.today.correctCount / selectedState.today.questionsAnswered) * 100) : 0}% accuracy</b><small>{progressNextAction}</small><button className="text-button" onClick={() => onStart(dailyMission.config)}>Start next repair <ArrowRight size={13} /></button></section>}
      ...
      {activeTab === "profile" && <>
        ...
        {isOwner && <details className="owner-diagram-desk tab-section"><summary><span><b>HELD DIAGRAM DESK</b><small>{ownerHeldDiagramRecords.length} figure-dependent records</small></span><ArrowRight size={16} /></summary><p className="owner-diagram-intro">Read-only safety queue. These records remain outside the learner bank until a neutral diagram can pass reconstruction and answer-safety review.</p><div className="owner-diagram-list">{ownerHeldDiagramRecords.length ? ownerHeldDiagramRecords.map((record) => <article key={record.id}><div><b>{record.subject} · {record.topic}</b><small>Record {record.id} · {record.externalId}</small><p>{record.questionPreview}</p></div><em>Held — reconstruction safety review required</em></article>) : <p className="compact-empty">No figure-dependent records are currently held for reconstruction safety review.</p>}</div></details>}
