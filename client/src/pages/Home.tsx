...
import type { OneSignalDiagnostic } from "@/lib/onesignal";
...
export default function Home({ loading, loadError, progress, canReview, onRetryLoad, onStart, auth, questionCount, questionCountReady, comeback, reminder, achievementStats, examHistory, onOpenExamLog = () => undefined, examReviewOpening = false, examReviewError = null, weakTopics, subjectPerformance = [], fullMockSubjectPerformance = [], availableTopics = [], bookmarks = [], comparison, onUpdateDailyMinimum, onUpdateDailyGoal = () => undefined, onEnablePush, onDisablePush, onTestPush = () => undefined, onRefreshProviderQueue = () => undefined, queueRefreshing = false, pushWorking, pushStatus, oneSignalDiagnostic = null, pwa, resumableCbt = null, onResumeCbt = () => undefined, onDiscardResumableCbt = () => undefined }: HomeProps) {
    ...
    <div key={`content-${activeTab}`} className="tab-content page-shell tab-cinematic-entry tab-cinematic-content">
      ...
      {activeTab === "progress" && <>
        <section className="compact-progress-grid tab-section" aria-label="Progress tools">
          ...
          <CompactPanel eyebrow="02 / EXAM LOG" title="Open a saved CBT correction" note={`${cbtHistory.length} recent CBT attempt${cbtHistory.length === 1 ? "" : "s"} · reopen any one for full corrections`} tone="paper">
            ...
            {cbtHistory.length ? <><AccuracyLineChart rounds={cbtHistory} /><div className="compact-log-list">{cbtHistory.map((round) => { const accuracy = round.questionCount ? Math.round((round.correctCount / round.questionCount) * 100) : 0; return <article key={round.id}><span><b>{round.subject}</b><small>{new Date(round.completedAt).toLocaleString()} · {round.questionCount} questions · {Math.max(0, Math.round(round.durationSeconds / 60))} min</small></span><strong>{accuracy}%</strong><button className="text-button" onClick={() => onOpenExamLog(round.id)} disabled={examReviewOpening}>{examReviewOpening ? "Opening…" : "Open corrections"}</button></article>; })}</div>{examReviewError && <p className="compact-goal-error" role="alert">{examReviewError}</p>}</> : <p className="compact-empty">Complete a timed CBT mock to create an exam log you can reopen question-by-question.</p>}
      ...
      {activeTab === "profile" && <>
        ...
        <section className="notification-section tab-section"><div className="notification-copy"><BellRing size={24} /><div><h2>Three daily JAMB reminders</h2><p>Enable reminders on this device, approve the browser prompt, then run a test before relying on the nudges. Your unfinished daily system can prompt you at 7:00 am, 1:00 pm, and 7:00 pm Lagos time; each window is sent at most once.</p>{reminder?.providerEnabled && <p className="provider-queue-status"><CalendarCheck2 size={14} /> {reminder.providerQueue?.scheduledCount ?? 0} provider-scheduled reminders queued for the next {reminder.providerQueue?.horizonDays ?? 28} days{reminder.providerQueue?.nextScheduledAt ? ` · next: ${new Date(reminder.providerQueue.nextScheduledAt).toLocaleString([], { weekday: "short", hour: "numeric", minute: "2-digit" })}` : ""}.</p>}{pushFeedbackMessages[pushStatus] && <p className="push-feedback" role="status">{pushFeedbackMessages[pushStatus]}</p>}</div></div><div className="notification-actions">{auth.isAuthenticated ? reminder?.providerEnabled ? <><span className="notification-status"><CalendarCheck2 size={15} /> OneSignal queue ready on this device</span><button className="button button-outline" onClick={onTestPush} disabled={pushWorking}><Send size={15} /> {pushWorking ? "Testing" : "Send test"}</button><button className="button button-outline" onClick={onRefreshProviderQueue} disabled={queueRefreshing || pushWorking}><RotateCcw size={15} /> {queueRefreshing ? "Refreshing" : "Refresh schedule"}</button><button className="button button-push" onClick={onDisablePush} disabled={pushWorking}><BellOff size={15} /> Turn off</button></> : <button className="button button-push" onClick={onEnablePush} disabled={pushWorking}><BellRing size={15} /> {pushWorking ? "Checking OneSignal" : pushStatus === "provider-pending" ? "Retry OneSignal setup" : "Enable on this device"}</button> : <button className="button button-push" onClick={startLogin}><LogIn size={15} /> Save reminders with profile</button>}</div></section>
        {oneSignalDiagnostic && <aside className="onesignal-diagnostic tab-section" role="status"><b>ONE SIGNAL DEVICE CHECK</b><span>Browser permission: {oneSignalDiagnostic.permission}</span><span>SDK: {oneSignalDiagnostic.sdkReady ? "ready" : "not ready"}</span><span>Provider push: {oneSignalDiagnostic.optedIn ? "opted in" : "not opted in"}</span><span>Device subscription: {oneSignalDiagnostic.subscriptionId ? "created" : "missing"}</span>{oneSignalDiagnostic.error && <small>{oneSignalDiagnostic.error}</small>}</aside>}
        <OfflineStudyPackPanel pwa={pwa} />
        <details className="achievement-gallery tab-section"><summary><span><Award size={19} /> Achievements</span><strong>{achievementSummary.earned.length} / 50 earned</strong><small>{achievementSummary.next ? `Next: ${achievementSummary.next.label}` : "All achievement marks earned"}</small><ArrowRight size={16} /></summary><div className="achievement-category-key"><span>Practice</span><span>Consistency</span><span>Mastery</span><span>Exam</span></div><div className="achievement-grid">{achievementSummary.badges.map((badge) => <article className={`${badge.earned ? "earned" : "locked"} achievement-${badge.category}`} key={badge.key}><Award size={16} /><div><b>{badge.label}</b><small>{badge.note}</small>{!badge.earned && <em>{Math.min(badge.value, badge.target)} / {badge.target}</em>}</div></article>)}</div></details>
      </>}
