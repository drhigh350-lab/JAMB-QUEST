...
export function GameArcade({ onExit, onSelect }: { onExit: () => void; onSelect: (mode: ArcadeMode) => void }) {
  ...
  return <main className="game-arcade" aria-labelledby="game-arcade-title">
    ...
    <section className="game-arcade-hero"><div><span className="eyebrow">YOUR STUDY GAMES</span><h1 id="game-arcade-title">{profile.displayName ? `${profile.displayName}, pick a game.` : "Pick a game."}</h1><p>Answer real JAMB questions, make simple choices, and fix any wrong answers. Each new session draws a fresh mix from the approved bank. Your normal Practice and CBT stay separate.</p></div><div className="game-arcade-orbit" aria-hidden="true"><span>ENG</span><span>BIO</span><span>CHE</span><span>PHY</span><b>IQ</b></div></section>
    ...
    <section className="game-arcade-worlds" aria-label="JAMB Quest game modes">
      <article className="game-arcade-world expedition-world"><div className="game-world-icon"><Map size={24} /></div><span className="eyebrow">GAME 01 / STUDY MAP</span><h2>Study Expedition</h2><p>Pick a subject, answer questions, build map paths, and fix any mistakes.</p><div className="game-world-tags"><span>choose subject</span><span>build paths</span><span>fix mistakes</span></div><button className="button button-dark" onClick={() => onSelect("expedition")}>Open Study Map <ArrowRight size={16} /></button></article>
      <article className="game-arcade-world president-world"><div className="game-world-icon"><Building2 size={24} /></div><span className="eyebrow">GAME 02 / BUILD A COUNTRY</span><h2>President’s Desk</h2><p>In fictional Asterra, answer questions to earn funds, choose what to build, and fix misses before your next turn.</p><div className="game-world-tags"><span>earn funds</span><span>choose project</span><span>fix mistakes</span></div><button className="button button-dark" onClick={() => onSelect("president")}>Build Asterra <ArrowRight size={16} /></button></article>
      <article className="game-arcade-world archive-world"><div className="game-world-icon"><Archive size={24} /></div><span className="eyebrow">GAME 03 / COLLECT TOPICS</span><h2>The Great Archive</h2><p>Answer questions to collect the real topics you have learned and repair the ones you missed.</p><div className="game-world-tags"><span>collect topics</span><span>choose a goal</span><span>fix mistakes</span></div><button className="button button-dark" onClick={() => onSelect("archive")}>Open Archive <ArrowRight size={16} /></button></article>
    </section>
