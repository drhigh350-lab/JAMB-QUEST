import { ArrowRight, Archive, Building2, Map, ShieldCheck, Swords, X } from "lucide-react";
...
export function GameArcade({ onExit, onSelect, questions, onChallenge }: { onExit: () => void; onSelect: (mode: ArcadeMode) => void; questions: BankQuestion[]; onChallenge: () => void }) {
  ...
  return <main className="game-arcade" aria-labelledby="game-arcade-title">
    ...
    <section className="game-arcade-worlds" aria-label="JAMB Quest game modes">
      <article className="game-arcade-world challenge-world"><div className="game-world-icon"><Swords size={24} /></div><span className="eyebrow">GAME 04 / PLAY TOGETHER</span><h2>Challenge Mode</h2><p>Make a named quiz from real JAMB Quest questions, share it with friends, and race up its global leaderboard.</p><div className="game-world-tags"><span>name your quiz</span><span>share a code</span><span>global leaderboard</span></div><button className="button button-dark" disabled={questions.length < 5} onClick={onChallenge}>Create a Challenge <ArrowRight size={16} /></button></article>
