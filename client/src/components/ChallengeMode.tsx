...
export function ChallengeMode({ questions, onExit, initialCode = "" }: ChallengeModeProps) {
  ...
  const [challengeVisibility, setChallengeVisibility] = useState<"link_only" | "public">("link_only");
  ...
  return <main className="challenge-mode" aria-labelledby="challenge-mode-title">
    ...
    {screen === "home" && <>
      ...
      <section className="challenge-grid">
        <article className="challenge-card challenge-create-card"><span className="eyebrow">CREATE A CHALLENGE</span><h2>Name your battle.</h2><p>Choose between 5 and 30 questions from the approved JAMB Quest bank.</p><label>Challenge name<input value={challengeName} onChange={(event) => setChallengeName(event.target.value)} placeholder="Biology Boss Battle" maxLength={80} /></label><label>Short description (optional)<input value={challengeDescription} onChange={(event) => setChallengeDescription(event.target.value)} placeholder="A focused race through Genetics" maxLength={240} /></label><label>Who can find it?<select value={challengeVisibility} onChange={(event) => setChallengeVisibility(event.target.value as "link_only" | "public")}><option value="link_only">Link only — friends with the code</option><option value="public">Public Arena — show it in Discover</option></select></label><div className="challenge-question-picker"><div className="picker-head"><b>{selectedIds.length} questions picked</b><button onClick={() => setSelectedIds(questions.slice(0, 10).map((question) => question.id))}>Reset to 10</button></div><div className="picker-subjects" aria-label="Choose question subject">{pickerSubjects.map((subject) => <button key={subject} className={pickerSubject === subject ? "active" : ""} onClick={() => setPickerSubject(subject as "All" | BankQuestion["subject"])}>{subject}</button>)}</div><div className="picker-list">{pickerQuestions.map((question) => <button key={question.id} className={selectedIds.includes(question.id) ? "picked" : ""} onClick={() => toggleQuestion(question.id)}><span>{selectedIds.includes(question.id) ? "✓" : ""}</span><b>{question.subject}</b><small>{question.topic}</small></button>)}</div></div><button className="challenge-primary" disabled={createMutation.isPending || selectedIds.length < 5 || !challengeName.trim()} onClick={() => createMutation.mutate({ challengeName, questionIds: selectedIds, visibility: challengeVisibility, description: challengeDescription || undefined })}><Play size={16} /> Create named challenge</button>{challengeCode && <div className="challenge-share-box"><b>{challengeName || "Your challenge"}</b><strong>{challengeCode}</strong><button onClick={copyLink}><Copy size={15} /> Copy share link</button></div>}</article>
