...
export function ChallengeMode({ questions, onExit, initialCode = "" }: ChallengeModeProps) {
  const [screen, setScreen] = useState<ChallengeScreen>(initialCode ? "play" : "home");
  ...
  const creatorQuestions = useMemo(() => questions.filter((question) => selectedIds.includes(question.id)), [questions, selectedIds]);
