  ...
  return (
    <main className="quiz-layout page-shell">
      ...
      <div className="quiz-workspace question-palette-bottom">
        ...
        <div className="cbt-ledger-stack">{displayCbt && <div className="cbt-actions"><div className="cbt-nav-actions"><button className="button button-outline" onClick={navigatePrevious} disabled={!canGoPrevious}>Previous</button><button className="button button-outline" onClick={navigateNext} disabled={!canGoNext}>Next</button></div>{!historicalReview && <><button className={`button ${currentFlagged ? "button-primary" : "button-outline"}`} onClick={onToggleFlag}><Flag size={15} /> {currentFlagged ? "Unflag question" : "Flag for review"}</button><AlertDialog open={submitConfirmOpen} onOpenChange={setSubmitConfirmOpen}><AlertDialogTrigger asChild><button className="button button-dark">Finish & review <Send size={15} /></button></AlertDialogTrigger><AlertDialogContent className="cbt-finish-dialog"><AlertDialogHeader><AlertDialogTitle>Review before submission</AlertDialogTitle><AlertDialogDescription>Use the real JAMB-style review step before finalising this mock. Keyboard: R returns to the exam; Y confirms the review screen.</AlertDialogDescription></AlertDialogHeader><div className="cbt-submit-summary"><div><strong>{answeredCount}/{questions.length}</strong><span>answered</span></div><div><strong>{unansweredCount}</strong><span>unanswered</span></div><div><strong>{flaggedIds.length}</strong><span>flagged</span></div></div><div className="cbt-submit-review-actions"><button className="button button-outline" disabled={!unansweredCount} onClick={() => reviewMatching((question) => !answers[question.id])}>Review unanswered</button><button className="button button-outline" disabled={!flaggedIds.length} onClick={() => reviewMatching((question) => flaggedIds.includes(question.id))}>Review flagged</button></div><AlertDialogFooter className="cbt-finish-footer"><AlertDialogCancel className="cbt-finish-cancel">Continue exam</AlertDialogCancel><AlertDialogAction className="cbt-finish-action" onClick={() => { setSubmitConfirmOpen(false); onFinishCbt?.(); }}>Review my answers</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></>}</div>}
          <QuestionLedger questions={questions} currentIndex={currentIndex} answers={answers} cbtMode={historicalReview ? false : cbtMode} fullMock={fullMockCbt} flaggedIds={flaggedIds} onNavigate={onNavigate} />
        </div>
      </div>
    </main>
  );
