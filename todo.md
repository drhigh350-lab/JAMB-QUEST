...
- [ ] Replace the non-executing date-specific Heartbeat test method with a scheduler-safe controlled execution route, then obtain both callback evidence and a real device receipt before considering scheduled notifications fixed. A one-minute controlled test and a temporarily repointed existing afternoon job also produced no new executions. The uncertain afternoon job was paused before any stray send, deleted, and recreated cleanly as `jamb-quest-comeback-afternoon-lagos-v3` at 13:00 Africa/Lagos; the provider and phone direct tests remain working, so scheduler dispatch is the outstanding fault.
- [ ] Resolve the confirmed scheduler-dispatch fault: the user’s 10-minute wait after the controlled test produced no notification because no scheduled callback executed. Do not create further temporary jobs or claim delivery until a supported production scheduler route is verified.
...
- [ ] Repair and prove the missing OneSignal provider subscription before queuing messages: the provider returned `All included players are not subscribed` for the learner’s external ID, proving the visible manual test was falling back to browser VAPID. The app now explicitly calls `User.PushSubscription.optIn()` after login and automatically refreshes the queue only after the preference update is persisted; reopening the published app on the learner’s phone and a fresh provider probe are still required evidence.
- [ ] Create a controlled 2:00 p.m. Africa/Lagos OneSignal provider test only after a fresh subscription probe confirms the learner’s device is subscribed; preserve the regular 7:00 a.m., 1:00 p.m., and 7:00 p.m. queue unchanged and record the provider response plus device receipt.
...
- [ ] Repair the stale installed-app update path reported by the learner: their phone still renders the previous “Keep JAMB Quest in your pocket” panel after the offline-pack checkpoint, so guarantee the current deploy invalidates the old application shell without deleting the separately downloaded study pack.
- [ ] Repair the learner-reported reminder-enrollment loop: the Profile button repeatedly moves through “Setting up” / “Enable on this device” without settling on a completed provider subscription, queue count, or actionable error. Trace and make the state deterministic before asking the learner to retry.
...
- [ ] Retire the repeatedly failing OneSignal enrollment route from the learner-facing reminder flow and restore JAMB Quest’s proven direct VAPID browser-push delivery, paired with a reliable schedule that does not use the already non-dispatching callback route.
- [ ] Implement the user-selected direct-push reminder route with three managed Lagos-time daily triggers at 7:00 a.m., 1:00 p.m., and 7:00 p.m.; create a separate controlled direct-push test and verify real device receipt before marking the system reliable.
...
- [ ] Diagnose the 17 August 8:00 a.m. Africa/Lagos notification non-delivery, correct the permanent scheduled-callback configuration without disturbing the intended 7:00 a.m., 1:00 p.m., and 7:00 p.m. Lagos reminders, and retain actual execution evidence.

- [ ] Renew the current installed-app browser-push subscription after the scheduler executed and the provider accepted both the 1:09 p.m. and 7:00 p.m. sends without a visible phone alert; verify an on-device test after renewal before claiming delivery.

- [ ] Evaluate and, if the owner provides or enables it, integrate OneSignal push delivery for JAMB Quest with device-level delivery diagnostics and a verified scheduled reminder test.

- [ ] Replace the current browser-push route after the final production test was provider-accepted but visibly absent on the learner device; preserve the JAMB Quest notification copy in the provider-managed replacement.
