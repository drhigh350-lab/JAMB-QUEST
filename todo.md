...
- [x] Inventory the remaining 369 authorised needs-review records by structural, topic, answer, option, and explanation blocker: the deterministic pass found 369 initial holds, then reduced them to 25, and finally to five duplicate-option holds.
- [x] Apply only deterministic or source-preserving repairs that clear every release gate; 356 records were released through official-topic mapping, source-preserving explanation shortening, and status-only approval, while malformed duplicate-option cards stayed held.
- [x] Revalidate the needs-review remediation pass and report every released record and residual protected hold: the live audit reports 3,468 authorised playable records and 4,468 learner-facing questions including the 1,000 model bank; five records remain `needs_review` because of duplicate option text, and 352 other authorised records remain excluded for invalid option content.
