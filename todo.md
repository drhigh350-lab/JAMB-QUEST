...
- [ ] Investigate the two-record drift between the earlier 5,990 snapshot and the current 5,988 learner-visible API-derived baseline, then re-derive any still-held figure-dependent exclusions from the live learner API and source ledger; recover only exact owner-original visuals, with no generated reconstruction or blank-figure release. The prior “30 remaining figures” estimate is stale after the four later source-backed record restorations.
