...
export function inferVerifiedTopic(subject: TopicSubject, questionText: string) {
  ...
  if (subject === "Biology") {
    if (has(/tissue is composed|level of organization|largest organ|lenticel|basic.*unit of life/)) return "Living Organisms and Organization";
    if (has(/gene|allele|chromosome|meiosis|mitosis|inheritance|blood group|sex-linked|evolution/)) return "Genetics and Evolution";
    if (has(/use and disuse|homologous|fossil|vestigial|new species|fossilization|variation|height, weight|skin color|physical and functional unit of heredity|color blindness|haemophilia/)) return "Genetics and Evolution";
    if (has(/ecology|succession|savanna|food chain|ddt|pollution|brackish|desert|symbiosis|commensalism|carbon cycle|biome|mangrove|natural resource|tapeworm and.*host/)) return "Ecology";
    ...
    if (has(/fertili[sz]|reproductive|ovary|testes|uterus|pregnan|menstru|gamete|ovulation|hermaphrodite|twins|fruit|placentation/)) return "Reproduction";
    ...
    if (has(/respiratory|gaseous exchange|trachea|alveoli|submerged|lenticel/)) return "Respiration and Gas Exchange";
    ...
    if (has(/eye|retina|atlas|axis|bone|skeletal|vitamin d|joint|muscle|vertebrae|exoskeleton/)) return "Support and Movement";
    ...
    if (has(/kingdom|phylum|taxonomic|classification|hydra|arthropod|monera|fungi|bacteria|biological organization|organism division|maize belongs|scales, teeth|nares|vertebrate|paramecium|homodont|aves|mammal|flightless bird|frog.*larval/)) return "Classification and Diversity";
    if (has(/hepatic portal|diffusion|concentration gradient/)) return "Transport";
    if (has(/body temperature|homeostasis|myopia|taste|stimulus|aestivation|taxis|reflex|magistrate.*verdict/)) return "Coordination and Control";
